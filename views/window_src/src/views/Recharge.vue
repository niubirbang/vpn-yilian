...
const load_for_mock = (planKey) => {
  plans.value = [{ "id": 1, "key": "vip", "name": "VIP", "title": "VIP会员", "remark": "开通VIP，享会员特权", "recommend": true, "prices": [{ "plan_id": 1, "period": "month_price", "name": "月套餐", "amount": 100, "originalAmount": 500, "break": 80, "unitPrice": 3, "recommend": false }, { "plan_id": 1, "period": "quarter_price", "name": "季度套餐", "amount": 200, "originalAmount": 1000, "break": 80, "unitPrice": 2, "recommend": false }, { "plan_id": 1, "period": "year_price", "name": "年套餐", "amount": 25800, "originalAmount": 129000, "break": 80, "unitPrice": 71, "recommend": true }] }, { "id": 2, "key": "svip", "name": "SVIP", "title": "SVIP会员", "remark": "开通SVIP，享会员特权", "recommend": false, "prices": [{ "plan_id": 2, "period": "month_price", "name": "月套餐", "amount": 100, "originalAmount": 500, "break": 80, "unitPrice": 3, "recommend": false }, { "plan_id": 2, "period": "quarter_price", "name": "季度套餐", "amount": 200, "originalAmount": 1000, "break": 80, "unitPrice": 2, "recommend": false }, { "plan_id": 2, "period": "year_price", "name": "年套餐", "amount": 46800, "originalAmount": 234000, "break": 80, "unitPrice": 128, "recommend": true }] }]
  if (planKey) {
    for (let plan of plans.value) {
      if (plan.key == planKey) {
        currentPlan.value = plan
  ...
  payments.value = [{ "id": 1, "uuid": "EUKCdaF8", "payment": "FeiMaoWxpay", "name": "微信", "pay_id": "6", "icon": "https://open.weixin.qq.com/zh_CN/htmledition/res/assets/res-design-download/icon64_appwx_logo.png", "config": "{\"url\":\"http:\\/\\/8.216.128.222\",\"pk\":\"f8ddc32907efa510c5cf9dc729d4d391\",\"merchant_no\":\"7151610013\"}", "notify_domain": null, "handling_fee_fixed": null, "handling_fee_percent": null, "enable": 1, "sort": null, "created_at": 1721912552, "updated_at": 1721965100 }, { "id": 2, "uuid": "EUKCdaF1", "payment": "FeiMaoAlipay", "name": "支付宝", "pay_id": "5", "icon": "https://cdn4.iconfinder.com/data/icons/logos-and-brands/512/13_Alipay_logo_logos-512.png", "config": "{\"url\":\"http:\\/\\/8.216.128.222\",\"pk\":\"f8ddc32907efa510c5cf9dc729d4d391\",\"merchant_no\":\"7151610013\"}", "notify_domain": null, "handling_fee_fixed": null, "handling_fee_percent": null, "enable": 1, "sort": null, "created_at": 1721912552, "updated_at": 1721965134 }]
  if (payments.value.length > 0) {
    currentPayment.value = payments.value[0]
  }
}
  ...
  <div class="large-full recharge-bg">
    ...
    <div class="inner">
      <div class="plan-selector">
        ...
        >
          ...
          <img
            v-if="plan.key === 'svip'"
            src="@/assets/image/icon-svip.png"
          >
          <p>{{ plan.name }}</p>
          <p
            v-if="plan.recommend"
          ...
          <div class="line">
            <template v-if="currentPlan.key === 'vip'">
              ...
              <p
                class="bold"
                v-else
              >有效期至: {{ $DateFormat(vipExpiredAt) }}</p>
            </template>
            <template v-if="currentPlan.key === 'svip'">
              ...
              <p
                class="bold"
                v-else
              >有效期至: {{ $DateFormat(svipExpiredAt) }}</p>
            </template>
          </div>
        </template>
        <template v-else>
    ...
    <div class="footer shadow">
      <div class="left">
        <div class="line">
          <p>
            <span class="countdown">{{ String(duration_info.hour).padStart(2, '0') }}</span>
            <span> : </span>
            <span class="countdown">{{ String(duration_info.minute).padStart(2, '0') }}</span>
            <span> : </span>
            <span class="countdown">{{ String(duration_info.second).padStart(2, '0') }}</span>
          </p>
          <p class="countdown-remark">后恢复原价</p>
        </div>
        <div class="line">
          <p class="selected-plan-price">
            <span>{{ currentPrice?.name }}{{ currentPlan?.key?.toUpperCase() }}会员: </span>
            <span class="emphasize">¥</span>
            <span class="emphasize large bold">{{ $Money(currentPrice?.amount || 0) }}</span>
          </p>
        </div>
      </div>
      <div class="right">
